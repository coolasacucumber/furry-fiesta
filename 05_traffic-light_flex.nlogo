...
to go
  ...
  move-turns
    ...
    or
    (any? turnlights with [color = green] and
      (count eastcars + count westbikes + count eastbikes + count straights + count car-straights) >
      ((count southcars + count northcars + count turns)* 1.5))
or
        (any? straightlights with [color = green] and ((count eastcars + count westbikes + count eastbikes)* 1.5) < (count southcars + count northcars))
...
if random-float 100 < north-car-spawn
  ...
  ask northcars at-points [[1 1]]
  [set heading 90]

  ask northcars at-points [[16 1]][
    die]
end
...
@#$#@#$#@
<experiments>
  <experiment name="turn-light-flexible_" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
