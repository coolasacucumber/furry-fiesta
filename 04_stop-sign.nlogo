;tab for manually adjustable probabilites

breed [cars car]
breed [turncars turncar]
breed [outcars outcar]
breed [checkcars checkcar]
...
ask turtles [
 ...
 set-default-shape crashes "fire"
  ...
  set-default-shape stopsigns "stop"
  create-space
end
...
ask patches [
  ...
  create-stopsigns 1 [setxy 2 -3]

 ;  ask patches [ ;turning lane
  ;  if pxcor > -5 and pXcor < 1 and pycor = 0
   ;   [set pcolor red]]
...
ask southcars at-points [[0 -16]] [die] ;terminate
   ...
   to move-outcars
  ask outcars at-points [[1 -3]]
  [set count-down waiting-time
    set breed waitcars]
 ...
 ;probability of spawning new out-car
  if random-float 100 < out-car-spawning-prob [
    create-outcars 1 [ setxy 1 -16 set heading 0]]
end

 to move-waitcars ;how do i specify for bikes only, and not cars............ later
  ask waitcars [stay]
end
...
to move-checkcars
  ask checkcars [
    if random-float 100 < east-not-looking
    [set breed nolookcars]]

   ask checkcars [
    ifelse not any? bikes in-radius 3 and
    not any? other turtles with [pycor = 1 and pxcor > -5 and pxcor < 0] ;adjusting to cars in middle lane
 [fd 1
    set breed cars]
      [stop]]
 end
...
ask bikes at-points [[-16 -1] [16 -2]] [ ;get rid of bikes
    die]
if random-float 100 < bottom-lane-bike-spawn [;5% of adding new bike in upper lane
    create-bikes 1 [ setxy -16 -2 set heading 90]]
if random-float 100 < upper-lane-bike-spawn [;5% of adding new car in bottom lane
    create-bikes 1 [ setxy 16 -1 set heading 270]]
end
...
or (count nolookcars-here > 0 and count bikes-here > 0)
  ...
  [sprout-crashes 1
 ask cars-here [die]
ask turncars-here [die]
ask outcars-here [die]
ask checkcars-here [die]
...
;ideas
;implement signs
;implement larger blue area and specify that as waiting space
;formalize: what determines if outcar goes. should hit bikes
@#$#@#$#@
GRAPHICS-WINDOW
210
10
...
"" ""
PENS
"cars" 1.0 0 -16777216 true "" "plot count cars + count turncars + count checkcars + count waitcars + count nolookcars + count southcars"
...
@#$#@#$#@
<experiments>
  <experiment name="stop-sign_" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    ...
    <enumeratedValueSet variable="waiting-time">
      <value value="8"/>
    </enumeratedValueSet>
