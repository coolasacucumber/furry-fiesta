...
to setup ;lay-out
  ...
  set-default-shape tunnelbikes "default" ;pil
ask turtles [
...
ask patches [
 ...
 ask patches at-points [[2 -1] [2 -2] [-1 -1] [-1 -2]]
  [set pcolor grey - 2]
   ask patches at-points [[3 -1] [3 -2] [-2 -1] [-2 -2]]
  [set pcolor grey - 1.5]
...
to move-turncars
  ask turncars [set heading 180]
  ask turncars at-points [[0 0]][
...
to move-southcars
  ...
  ask southcars at-points [[0 -16]] [die] ;terminate
   ...
   to move-outcars ;possibly unnecessary
   ask outcars [
   if not any? turtles-on patch-ahead 1 ; if no turtle in front, go 1
     [fd 1]]
   ask outcars [
       if any? turtles-on patch-ahead 1 ; if turtle ahead, stop
     [stop]]
 ...
 to move-waitcars
   ...
   ask waitcars
 [ifelse not any? other turtles with [pycor = 1 and pxcor > -5 and pxcor < 0]
    [fd 1
  set breed cars]
    [stop]]
 end
...
to move-bikes
  ...
  ask bikes at-points [[-2 -2] [3 -1]]
  [set breed tunnelbikes]
...
to move-tunnelbikes
  ask tunnelbikes at-points [ [-1 -2] [1.5 -1]] [
    hide-turtle]
  ask tunnelbikes at-points [[-1 -1] [1.5 -2]] [
    show-turtle]

  ask tunnelbikes at-points [[3 -2] [-2 -1]]
  [set breed bikes]
...
;; crashes ;;
;;;;;;;;;;;;;
;; not actually appplicable in this condition ;;
...
"" ""
PENS
"cars" 1.0 0 -16777216 true "" "plot count cars + count turncars + count outcars + count waitcars + count southcars"
"bikes" 1.0 0 -7500403 true "" "plot count bikes + count tunnelbikes"
...
@#$#@#$#@
<experiments>
  <experiment name="tunnel_" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="293333"/>
    <metric>count turtles</metric>
    <metric>count bikes</metric>
    <metric>count tunnelbikes</metric>
