...
ask patches [
  ...
  ask patches [ ;road
    if pycor = 2 or pycor = 1 or pycor = 0 or ((pxcor = 1) and (pycor < 1)) or ((pxcor = 0) and (pycor < 1)) [
...
ask bikes at-points [[-16 -1] [16 -2]] [ ;get rid of bikes
    die]
if random-float 100 < upper-lane-bike-spawn [;5% of adding new bike in upper lane
    create-bikes 1 [ setxy -16 -2 set heading 90]]
if random-float 100 < bottom-lane-bike-spawn [;5% of adding new car in bottom lane
...
@#$#@#$#@
<experiments>
  <experiment name="east-not-looking" repetitions="5" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    ...
    <steppedValueSet variable="east-not-looking" first="0" step="5" last="100"/>
  ...
  <experiment name="set probabilities" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="50000"/>
    <metric>count graves</metric>
